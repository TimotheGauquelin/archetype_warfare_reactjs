const switchEngToFrenchType = (type: string): string | null => {
  var newType;
  switch (type) {
    case "Aqua":
      newType = "Aqua";
...
const switchEngToFrenchAttribute = (attribute: string): string | null => {
  var newAttribute: string | null;
  switch (attribute) {
    case "DARK":
...
interface CardWithTypeAndLevel {
  cardType: { label: string };
  level: number;
  name: string;
}

const sortingCardsFromCardTypeLabelAndLevelAndName = (cardTypes: string[], a: CardWithTypeAndLevel, b: CardWithTypeAndLevel): number => {
  return (
...
const convertJavaDateIntoJS = (javaDate: string | Date): string => {
