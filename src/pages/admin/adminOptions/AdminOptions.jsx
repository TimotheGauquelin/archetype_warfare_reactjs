...
const AdminOptions = () => {
  ...
  const handleToggleStreamBar = useCallback(() => {
    ...
    setIsUpdating(true);
    toggleStreamBar(token, setIsUpdating, toast)
  }, [token, isUpdating]);

  const handleToggleRegistration = useCallback(() => {
    if (!token || isUpdating) return;
    setIsUpdating(true);
    toggleRegistration(token, setIsUpdating, toast)
  }, [token, isUpdating])
        ...
        <>
          <div className="flex items-center justify-between p-4 mb-2 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <SwitchInput
              ...
              data={config}
              setAction={handleToggleStreamBar}
              disabled={isUpdating}
            />
          </div>
          ...
          <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <SwitchInput
              ...
              data={config}
              setAction={handleToggleRegistration}
              disabled={isUpdating}
            />
          </div>
        </>
