...
interface CheckboxInputProps<T = Record<string, unknown>> {
  label: string;
  required?: boolean;
  options: Option[];
  colSpanWidth?: string;
...
function CheckboxInput<T extends Record<string, unknown>>({
  label,
  required,
  options,
  attribute,
    ...
    <div className="flex flex-col">
      ...
      <div className="flex grid grid-cols-12">
        {options.map((op, index) => {
          const attributeValue = data[attribute] as Option[] | undefined;
          const isChecked = Array.isArray(attributeValue) && attributeValue.some(
            (element: Option) => element.id === op.id
          );
            ...
            >
              <input
                ...
                onChange={(e) => {
                  ...
                  setAction((prevState) => {
                    const currentValue = prevState[attribute] as Option[] | undefined;
                    const updateElement = [...(currentValue || [])];
